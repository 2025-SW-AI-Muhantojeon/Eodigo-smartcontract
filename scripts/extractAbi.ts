...
const artifactPath = path.join(
  __dirname,
  "../artifacts/contracts/core/Gift.sol/Gift.json"
);
...
const abi = json.abi;
fs.writeFileSync(
  path.join(abiFolder, "Gift.abi"),
  JSON.stringify(abi, null, 2)
);
console.log("ABI saved to:", path.join(abiFolder, "Gift.abi"));

// BIN 추출
const bin = json.bytecode;
fs.writeFileSync(path.join(binFolder, "Gift.bin"), bin);
console.log("BIN saved to:", path.join(binFolder, "Gift.bin"));
